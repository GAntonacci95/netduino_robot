...
namespace NetduinoPlusSide.Moduli
{
    namespace Videocamera
    {
        ...
        public class Videocamera
        {
            ...
            private static AdaFruitVC0706 cam;
            private static string port;
            private static XBeeExecutor client;

            public AdaFruitVC0706 Camera { get { return cam; } }
            public string Porta { get { return port; } }
            public static bool Init { get { return port != ""; } }

            public static void Inizializza(ref XBeeExecutor asymodclient, int refreshtime,
                string porta, PortSpeed baud, ImageSize size)
            {
                client = asymodclient;
                //while (!asymodclient.Connesso) ;
                cam = new AdaFruitVC0706();
                refresh = refreshtime;
                port = porta;

                if (Init && client.Connesso)
                {
                    cam.Initialize(port, baud, size, ref asymodclient);
                    cam.TvOutput(false);
            ...
            private static void Acquisizione()
            {
                while (client.Connesso)
                {
                    ...
                    catch (System.Net.Sockets.SocketException) { Thread.CurrentThread.Abort(); }
                    Thread.Sleep(refresh);
                }
            }
        }

        public class FrameVideocamera : Frames.FrameInUscita
        {
            public class CaratteristicheFrameVideocamera : CaratteristicheFrameInUscita
            {
                ...
                public string StreamVideocameraBytesToString { get { return NAndCoder.Other.GetEachByteString(this.streamvideo); } }
            ...
            public FrameVideocamera(int num, byte[] bufferread)
            {
                base.Caratteristiche.Tipo = this.Caratteristiche.Tipo = Frames.TipoFrame.cam;
                this.Caratteristiche.Numero = num;
                this.Caratteristiche.StreamVideocamera = bufferread;
                SubBuild();
                Build();
            }
            ...
            protected void SubBuild()
            {
                base.Caratteristiche.Testo = InitFine + Separatore + Frames.EnumExt.GetName(base.Caratteristiche.Tipo)
                    + Separatore + this.Caratteristiche.Numero.ToString() + Separatore
                    + this.Caratteristiche.StreamVideocameraBase64 + Separatore + InitFine;
            }
        }
        ...
        public class AdaFruitVC0706 : IDisposable
        {
            ...
            private const int _bufferSize = 60;
            private byte[] _frameBuffer = new byte[_bufferSize];

            public void Initialize(string port, PortSpeed baudRate, ImageSize imageSize, ref XBeeExecutor asymodclient)
            {
                client = asymodclient;
            ...
            public void TakePicture()
            {
                ...
                int multi = 1;
                byte[] tmp = new byte[multi * _bufferSize];
                int t = 0;
                int nframe = 0;
                ...
                while (frameLength > 0)
                {
                    ...
                    {
                        if (t < (multi * _bufferSize))
                        {
                            if (frameLength <= _bufferSize)
                            {
                                client.Send(new FrameVideocamera(-1, tmp));
                                t = 0;
                                nframe = 0;
                                tmp = new byte[multi * _bufferSize];
                            }
                            else
                            {
                                tmp[t] = _frameBuffer[i];
                                t++;
                            }
                        }
                        else if (t == (multi * _bufferSize))
                        {
                            client.Send(new FrameVideocamera(nframe, tmp));
                            t = 0;
                            nframe++;
                            tmp = new byte[multi * _bufferSize];
                        }
